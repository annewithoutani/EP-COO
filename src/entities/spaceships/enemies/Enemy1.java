...
public class Enemy1 extends Enemy {
    ...
    public void updateState(long delta, long currentTime, List<Projectile> eprojectiles1) {
        if (getState() == Main.EXPLODING) {
            handleExplodingState(currentTime);
            return;
        }
        if (getState() == Main.ACTIVE) {
            handleActiveState(delta, eprojectiles1);
        }
    }
    ...
    private void handleActiveState(long delta, List<Projectile> eprojectiles1) {
        if (isOutOfScreen()) {
            setState(Main.INACTIVE);
            return;
        }
        ...
        if (shoot) {
            shootProjectiles(eprojectiles1);
        }
    }
    ...
    private void shootProjectiles(List<Projectile> eprojectiles1) {
        double[] angles = { Math.PI / 2 + Math.PI / 8, Math.PI / 2, Math.PI / 2 - Math.PI / 8 };
        int[] freeArray = Main.findFreeIndex((java.util.ArrayList<? extends entities.Entity>) eprojectiles1, angles.length);
        for (int k = 0; k < freeArray.length; k++) {
            ...
            if (free < eprojectiles1.size()) {
                ...
                eprojectiles1.get(free).setVX(vx * 0.20);
                eprojectiles1.get(free).setVY(vy * 0.20);
