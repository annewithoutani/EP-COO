...
public class Enemy3 extends Enemy {
    ...
    public void updateState(long delta, long currentTime, List<Projectile> eprojectiles3) {
        if (getState() == Main.EXPLODING) {
            handleExplodingState(currentTime);
            return;
        }
        if (getState() == Main.ACTIVE) {
            handleActiveState(delta, eprojectiles3);
        }
    }
    ...
    private void handleActiveState(long delta, List<Projectile> eprojectiles3) {
        if (isOutOfScreen()) {
            setState(Main.INACTIVE);
            return;
        }
        ...
        if (shoot) {
            shootProjectiles(eprojectiles3);
        }
    }
    ...
    private void shootProjectiles(List<Projectile> eprojectiles3) {
        double[] angles = { Math.PI / 2 + Math.PI / 8, Math.PI / 2, Math.PI / 2 - Math.PI / 8 };
        int[] freeArray = Main.findFreeIndex((java.util.ArrayList<? extends entities.Entity>) eprojectiles3, angles.length);
        for (int k = 0; k < freeArray.length; k++) {
            ...
            if (free < eprojectiles3.size()) {
                ...
                eprojectiles3.get(free).setVX(vx * 0.20);
                eprojectiles3.get(free).setVY(vy * 0.20);
